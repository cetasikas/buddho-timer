<div align="center">

<img src="logo.png" alt="Buddho Timer Logo" width="96" height="96">

# Buddho Timer
### A Distraction-Free Meditation Timer & Mindful Breathwork Companion

[![Website](https://img.shields.io/badge/Website-cetasikas.github.io%2Fbuddho--timer-D4AF37?style=flat-square)](https://cetasikas.github.io/buddho-timer/)
[![Privacy Policy](https://img.shields.io/badge/Privacy-100%25_Offline-0E1420?style=flat-square&logo=shield)](https://cetasikas.github.io/buddho-timer/privacy.html)
[![Terms of Service](https://img.shields.io/badge/Terms-Free_to_Use-1E293B?style=flat-square)](https://cetasikas.github.io/buddho-timer/terms.html)
[![Ko-fi](https://img.shields.io/badge/Support_on-Ko--fi-FF5E5B?style=flat-square&logo=kofi&logoColor=white)](https://ko-fi.com/hackysackapp)

<br>

<img src="feature_graphic.png" alt="Buddho Timer App Preview" width="100%" style="border-radius: 12px; box-shadow: 0 8px 30px rgba(0,0,0,0.5);">

</div>

---

## 🌿 Quick Links

* 🌟 **Live Landing Page:** [https://cetasikas.github.io/buddho-timer/](https://cetasikas.github.io/buddho-timer/)
* 🔒 **Privacy Policy:** [https://cetasikas.github.io/buddho-timer/privacy.html](https://cetasikas.github.io/buddho-timer/privacy.html)
* 📜 **Terms of Service:** [https://cetasikas.github.io/buddho-timer/terms.html](https://cetasikas.github.io/buddho-timer/terms.html)
* ☕ **Support on Ko-fi (Dāna):** [https://ko-fi.com/hackysackapp](https://ko-fi.com/hackysackapp)

---

## ✨ Core Features

* 🔔 **Authentic Acoustic Chimes**: High-fidelity recordings of mindfulness bells, deep monastery bells, and Tibetan singing bowls with natural reverberation.
* 🫁 **Mindful Box Breathing**: Built-in Pranayama pacing (*Inhale, Hold, Exhale, Hold*) with rhythmic visual cues and configurable intervals.
* 📳 **Silent Haptics**: Multi-stage tactile vibration waveforms for quiet meditation in shared or silent retreat settings.
* 🌑 **Distraction-Free Interface**: Deep obsidian dark mode for zero eye strain, gentle falling leaf animations, and tap-to-blackout display.
* 📊 **Practice Journal & Streaks**: Track your daily practice consistency and meditation milestones in a local SQLite database.
* 🔒 **100% Offline & Private**: Zero user tracking, zero advertisements, no user accounts, and no network permissions (`android.permission.INTERNET` is completely absent).
* 💾 **Data Sovereignty**: Manual JSON/CSV backup export and automatic Google Drive sandbox backup support.

---

## 🔒 Privacy Guarantee

Buddho Timer is built with a strict **privacy-first philosophy**:
* **No Telemetry**: We do not include Google Analytics, Firebase, or tracking frameworks.
* **No Cloud Accounts**: Everything resides securely on your own device.
* **Zero Monetization**: Free for all practitioners without paywalls.

For full details, please review our [Privacy Policy](https://cetasikas.github.io/buddho-timer/privacy.html).

---

## ☕ Support the Project

Buddho Timer is provided **100% free with no paywalls or ads**. 

If this application enriches your daily mindfulness practice, you are welcome to offer voluntary support (Dāna):

👉 **[Support on Ko-fi](https://ko-fi.com/hackysackapp)**

---

<div align="center">
  <sub>© 2026 Buddho Timer. Built with intention for mindful practice.</sub>
</div>
